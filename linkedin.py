...
class AutoApplyLinkedin: 
    ...
    def job_search(self):
        ...
        #enters key word and location, clicks search
    ...
    def find_offers(self):
        #finds offers compatable with east apply
        total_results = self.driver.find_element_by_class_name("display-flex t-12 t-black--light t-normal")
        total_results_int = int(total_results.text.split(' ',1)[0].replace(",",""))
        print(total_results_int)
        time.sleep(2)
        current_page=self.driver.current_url
        results=self.driver.find_element_by_class_name("occludable-update.artdeco-list__item--offset-4.artdeco-list__item.p0.ember-view")

        #for each job, submit appliation if no questions are asked 
        for result in results:
            hover= ActionChains(self.driver).move_to_element(result)
            hover.perform()
            titles=result.find_element_by_class_name("job-card-search__title.artdeco-entity-lookup__title.ember-view")
            for title in titles: 
                self.submit_application(title)

        #if there is more than 1 page, find the next page and keep applying
        if total_result_int >24: 
            time.sleep(2)
            #find the last page and construct a URL 
            find_pages = self.driver.find_elements_by_class_name("artdeco-pagination__indicator.artdeco-pagination__indicator--number")
            total_pages=find_pages[len(find_pages)-1].text
            total_pages_int = int(re.sub(r"[^\d.]", "", total_pages))
            get_last_page = self.driver.find_element_by_xpath("//button[@aria-label='Page "+str(total_pages_int)+"']")
            ...
            #go to all available pages and job offers and apply
            for page_number in range(25,total_jobs+25):
                self.driver.get(current_page+'&start='+str(page_number))
                time.sleep(2)
                results_ext = self.driver.find_elements_by_class_name("occludable-update.artdeco-list__item--offset-4.artdeco-list__item.p0.ember-view")
                for result_ext in results_ext:
    ...
    def submit_application(self,job_ad):
        ...
        try:
            ...
            print('You already applied to this job, going to next...')
            pass
        time.sleep(1)

        try: 
            submit=self.driver.find_element_by_xpath("//buttin[@data-control-name='submit_unify']")
            submit.send_keys(Keys.RETURN)

        except NoSuchElementException:
                print('Not a direct application.. Going to next')
                try: 
                    discard=self.driver.find_element_by_xpath("//button[@data-text-modal-close-btn]")
                    discard.send_keys(Keys.RETURN)
                    time.sleep(1)
                    discard_confirm=self.driver.find_element_by_xpath("//button[@data-test-dialog-primary-btn]")
                    discard_confirm.send_keys(Keys.RETURN)
